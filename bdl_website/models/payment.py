...
class PaymentAcquirerPONumber(models.Model):
    ...
    provider = fields.Selection(selection_add=[('po_number', 'PO Number')],
                                ondelete={'po_number': 'set default'})
...
class AccountPayment(models.Model):
    ...
    @api.onchange('line_ids', 'payment_method_id')
    def onchange_invoice_ids(self):
        result = {'domain': {}, 'value': {}, 'warning': {}}
        if self.line_ids:
            if self.payment_method_code == 'electronic' and len(self.line_ids.mapped('partner_id')) > 1:
                raise UserError(
                    _("You can not process payments for invoices belonging to multiple customers electronically."))
            partners = self.line_ids.mapped('partner_id') | self.line_ids.mapped(
                'partner_id.commercial_partner_id') | self.line_ids.mapped(
